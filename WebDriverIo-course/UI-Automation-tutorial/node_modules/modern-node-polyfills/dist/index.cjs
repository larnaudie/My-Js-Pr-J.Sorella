...
var _chunkJWDSS33Ocjs = require('./chunk-JWDSS33O.cjs');
...
function inject(options) {
  ...
  if (_path.sep !== "/") {
    modulesMap.forEach((mod, key) => {
      modulesMap.set(key, Array.isArray(mod) ? [mod[0].split(_path.sep).join("/"), mod[1]] : mod.split(_path.sep).join("/"));
    });
  }
  ...
  return {
    ...
    transform(code, id) {
      ...
      let scope = _pluginutils.attachScopes.call(void 0, ast, "scope");
      ...
      function handleReference(node, name, keypath) {
        ...
        if (mod && !imports.has(name) && !scope.contains(name)) {
          ...
          const importLocalName = name === keypath ? name : _pluginutils.makeLegalIdentifier.call(void 0, `$inject_${keypath}`);
...
if (!globalThis.require) {
  const require2 = _module.createRequire.call(void 0, import.meta.url);
...
async function polyfillPath(mod) {
  ...
  if (!_module.builtinModules.includes(mod))
    throw new Error(`Node.js does not have ${mod} in its builtin modules`);
  const jspmPath = _path.resolve.call(void 0, _chunkJWDSS33Ocjs.__require.resolve(`@jspm/core/nodelibs/${mod}`), "../../.." + (mod.includes("/") ? "/.." : ""));
  const jspmPackageJson = await _localpkg.loadPackageJSON.call(void 0, jspmPath);
  const exportPath = resolve(jspmPackageJson, `./nodelibs/${mod}`, {
    browser: true
  });
  const exportFullPath = _localpkg.resolveModule.call(void 0, _path.join.call(void 0, jspmPath, exportPath || ""));
...
async function polyfillContent(mod) {
  ...
  const content = (await _esbuild.build.call(void 0, {
    write: false,
    format: "esm",
    bundle: true,
...
async function inject2(content, options, id) {
  ...
  const parse = (input, opts) => _acorn.parse.call(void 0, input, { ...opts, sourceType: "module", ecmaVersion: "latest" });
  const result = injectPlugin.transform.call({ warn: console.warn, parse }, content, _nullishCoalesce(id, () => ( "")));
  return _optionalChain([result, 'optionalAccess', _2 => _2.code]) || content;
}
async function polyfillGlobals(content, globals = {}, mods = {}) {
  return inject2(content, {
    expressions: {
      __filename: _optionalChain([globals, 'optionalAccess', _3 => _3.__filename]) || "/",
      __dirname: _optionalChain([globals, 'optionalAccess', _4 => _4.__dirname]) || "/"
    },
    modules: removeEmpty({
      process: await polyfillPath("process"),
      Buffer: [await polyfillPath("buffer"), "Buffer"],
      global: _chunkJWDSS33Ocjs.__require.resolve("modern-node-polyfills/global"),
...
exports.inject = inject2; exports.polyfillContent = polyfillContent; exports.polyfillGlobals = polyfillGlobals; exports.polyfillPath = polyfillPath;
